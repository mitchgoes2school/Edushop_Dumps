<?php
    ...
    if(isset($_FILES['files']) && isset($_POST['sellBtn'])){
        ...
        if(isset($_POST['payment'])){$itemMOP = $_POST['payment'];}
        $files =  $_FILES;
        if($files['files']['name'][0] == ''){
            ...
            $bool = addRecord('items', ['user_id', 'item_title', 'item_description', 'item_price', 'item_quantity', 'item_condition', 'item_mode_of_payment'], [$user_id, $itemTitle, $itemDescription, $itemPrice, $itemQuantity, $itemCondition, $itemMOP]);
            $item = getLatest('items', 'item_id');
            $itemId = $item['item_id'];
            $path = "./users/{$user_id}/items/{$itemId}/";
            if($bool){
    ...
    if(isset($_FILES['files']) && isset($_POST['auctionBtn']) ){
        $user = $_SESSION['auth_user'];
        $user_id = $user['user_id'];
        $user_name = $user['user_first_name'];
        $itemTitle = $_POST['itemTitle'];
        ...
        if(isset($_POST['category'])){$itemCategory = $_POST['category'];}
        if(isset($_POST['list-type'])){$listType = $_POST['list-type'];}
        if(isset($_POST['payment2'])){$itemMOP = $_POST['payment2'];}
        $auctionItemDay = $_POST['auction_item_hours_duration'];
        $auctionItemHours = $_POST['auction_item_hours_duration'];
        $auctionItemType = $_POST['auction_item_type'];
        $auctionItemOpeningBid = $_POST['auction_item_opening_bid'];
        $files =  $_FILES;
        if($files['files']['name'][0] == ''){
            echo "<script>alert('Please select atleast 1 photo')</script>";
        }
            ...
            $tmpName = $files['files']['tmp_name'];
            $filesArray = array_combine($tmpName, $names);
            $bool = addRecord('items', ['user_id', 'item_title', 'item_description', 'item_price', 'item_quantity', 'item_condition','item_mode_of_payment', 'item_is_auctioned'], [$user_id, $itemTitle, $itemDescription, $itemPrice, $itemQuantity, $itemCondition, $itemMOP, $itemIsAuctioned]);
            $item = getLatest('items', 'item_id');
            $itemId = $item['item_id'];
            $path = "./users/{$user_id}/items/{$itemId}/";
            $record = addRecord('auction_item', ['items_id', 'user_id', 'auction_item_type', 'auction_item_day_duration', 'auction_item_hours_duration', 'auction_item_opening_bid'],[$itemId, $user_id, $auctionItemType, $auctionItemDay, $auctionItemHours, $auctionItemOpeningBid]);
            if($bool && $record){
                foreach($filesArray as $tmpFolder => $image_name){
                $fileName = $image_name."-".date("Ymd_His");
                    if (!is_dir($path)) {
...
<body>
    ...
    <div>
            ...
            <div class="showMenu h-screen w-full hidden">
                ...
                if(isset($_SESSION['authenticated'])){
                    if($_SESSION['authenticated']==TRUE){
                        ...
                        $response = getRecord('user_details','user_id',$user['user_id']);
        ...
        <div class="py-2" >
           ...
           <nav class="flex justify-center">
            <ul class="flex md:gap-40 gap-10 font-bold">
                ...
                <li class="hover:text-orange-500"><a class="text-orange-500" href="sell.php">SELL</a></li>
        ...
        <div class="main md:ml-32 md:mr-32 ml-5 mr-5">
            <div class="py-3">
                <h3 class="font-bold text-black" id="title">Sell</h3>
            </div>
            <div class="border-t-2 border-gray-500">
                ...
                <form action="sell.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex md:flex-row flex-col gap-2">
                        ...
                        <input class="rounded-md pl-2 border-2 border-gray-400"type="number" name="itemPrice" id="price" placeholder="Price" required>
                        <input class="rounded-md pl-2 border-2 border-gray-400"type="number" name="quantity" id="quantity" placeholder="Quantity" required>
                    ...
                    <textarea id="Description" name="description" id="Description" placeholder="Description" rows="4" class="block p-2.5 bg-gray-100 border-gray-300 " placeholder="Item Description" style="width: 50%; margin-top: 20px;" required></textarea>
                    ...
                    <div id="auction" class="border-t-2 border-gray-500 mt-5 pt-2" style="display: none;">
                        <span>Time Duration of Auction</span>
                        <br>
                        <div class="flex md:flex-row flex-col gap-2">
                            <input class="rounded-md pl-2 border-2 border-gray-400"type="number" placeholder="Time duration in days" name="auction_item_day_duration">
                            <label><span>:</span></label>
                            <input class="rounded-md pl-2 border-2 border-gray-400"type="number" placeholder="Time duration in hours" name="auction_item_hours_duration">
                        </div>
                        <br>
                        <select class="rounded-md pl-2 border-2 border-gray-400" name="auction_item_type" id="auctionitem" >
                            <option hidden>Select a type of the item</option>
                            <option value="collectible">Collectible Item</option>
                            <option value="academic">With Academic Value</option>
                        </select>
                        <br>
                        <br>
                        <input class="rounded-md pl-2 border-2 border-gray-400"type="number" placeholder="Opening Bid" name="auction_item_opening_bid">
                        <br>
                        <br>
                        <span>Mode of Payment</span>
                        <br>
                        <input type="radio" name="payment2" value="Gcash"><span class="ml-2">Gcash</span>
                        <input type="radio" name="payment2" value="Meet-up"><span class="ml-2">Meet-up</span>
                        <br>
                        <br>
                        <div style="text-align: right;">
                            <button type="submit" name="auctionBtn" class="auctionBtn w-24 h-10 border-2 border-orange-500 text-white bg-orange-500 rounded-3xl font-bold hover:bg-gray-50 hover:text-orange-500 mt-2 shadow-lg drop-shadow-lg" id="auctionBtn">Auction</button>
                        </div>
                    </div>
                </form>
           </div>
